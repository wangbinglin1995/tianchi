...
class cnn_inception():
    ...
    def get_data(self, Data, is_test=True):
        ...
        return x_train, y_train, gen1


    def train(self):

        d_train, d_eval = self.load_data()
        _, _, gen_train = self.get_data(d_train, False)
        _, _, gen_eval = self.get_data(d_eval, False)
        ...
        for i in range(self.improve):
            x_train, y_train = gen_train.next()
            x_eval, y_eval = gen_eval.next()
            history = LossHistory()
...
if __name__ == '__main__':
    ...
    x_pre, y_test, _ = cnn1.get_data(cnn1.d_predict)
