...
class cnn_inception():
    ...
    def load_data(self):
        ...
        self.d_train = Data[:train_num, :, :, :], Labels[:train_num]
        self.d_eval = Data[train_num:, :, :, :], Labels[train_num:]
    ...
    def get_data(self, Data):
        ...
        img_rows, img_cols = self.re_size
        img_cha = 3
    ...
    def train(self):
        ...
        base_model = self.model_type(include_top=False, weights='imagenet')
        model = base_model
        ...
        # optimizer='rmsprop'
        optimizer=keras.optimizers.SGD(lr=self.lr, decay=self.lr/20, momentum=0.9, nesterov=True)

        model.compile(loss=keras.losses.categorical_crossentropy, optimizer=optimizer, metrics=['accuracy'])

        x_train, y_train = self.get_data(self.d_train)
        x_eval, y_eval = self.get_data(self.d_eval)
    ...
    def retrain(self, model):
        ...
        model2 = self.model_cnn()
        model.layers[6].set_weights(model2.layers[6].get_weights())
        model.layers[8].set_weights(model2.layers[8].get_weights())
    ...
    def predict(self, model, x_predict):        
        ...
        fo = open("result_inception_v4.csv", "a")
